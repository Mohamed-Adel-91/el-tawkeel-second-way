                                    ...
                                    <div class="col-md-2 d-flex justify-content-end p-0">
                                        <div class="col-md-6 d-flex justify-content-end  p-0">
                                            <button type="button" class="btn btn-primary" style="margin-top: 20px;">
                                                <a href="{{ route('admin.banks.create') }}" style="color: #fff;">
                                                    <i class="icon-plus-circle mr-1"></i>{{ __('admin.table.new') }}</a></button>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            @include('admin.partials.results-summary', ['data' => $data, 'label' => 'بنك'])
                                    ...
                                    <tbody>
                                        @if (!empty($data) && count($data) > 0)
                                            @foreach ($data as $item)
                                                ...
                                                <a href="{{ route('admin.banks.edit', $item->id) }}"
                                                                class="icon bg-info" data-toggle="tooltip"
                                                                data-placement="top" title="Edit Row">
                                                                <i class="icon-edit"></i>
                                                            </a>
                                                            {{-- <form method="POST" id="delete_form_{{ $item->id }}" class="d-inline delete_form" action="{{ route('admin.banks.destroy', ['bank' => $item->id]) }}">
                                                                @csrf
                                                                @method('DELETE')
